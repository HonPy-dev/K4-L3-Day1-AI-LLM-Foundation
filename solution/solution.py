...
def run_assistant(
    ...
    while True:
        ...
        num_turns += 1
        turn_cost_info = estimate_cost(user_msg, reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        g4 = (
            (r["gpt4o_response"][:37] + "...")
            if len(r["gpt4o_response"]) > 40
            else r["gpt4o_response"]
        )
        m = (
            (r["mini_response"][:37] + "...")
            if len(r["mini_response"]) > 40
            else r["mini_response"]
        )
        g4_l = f"{r['gpt4o_latency']:.2f}s"
        m_l = f"{r['mini_latency']:.2f}s"
        lines.append(f"{p:<40} | {g4:<40} | {m:<40} | {g4_l:<15} | {m_l:<15}")
    return "\n".join(lines)
